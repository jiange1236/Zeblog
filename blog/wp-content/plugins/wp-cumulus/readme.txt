...
WP-Cumulus 用3D的形式显示你的标签和/或者分类。

== Description ==

WP-Cumulus将你的标签、分类或者两者一起用3D flash的形式显示出来。 它跟普通的标签云原理相似，但是更加美观。

flash源代码可在wordpress.org上下载。

== Installation ==

= Installation =
1. 请确认你的WordPress 版本为 2.3或者更高。低于2.3版本将不能正常运行。
1. 下载zip文件，并解包。
1. 将 'wp-cumulus'文件夹上传到插件目录(wp-content/plugins/).
1. 在插件管理页面启用插件。
1. 在 '设置->WP Cumulus' 中进行。

=要让标签云正常显示，你需要做3种设置. =
1. 建立一个页面或文章，正文中输入 [wp-cumulus] 。当访问这个页面时，这个标签会被flash代替。参见 [here](http://www.roytanck.com/2009/03/11/how-to-use-wp-cumulus-shortcodes/) 。
1. 在主题中要显示标签云的地方添加代码：`<?php wp_cumulus_insert(); ?>`，这样你就可以在侧边栏使用WP Cumulus（侧边栏的宽度可能会小于最佳显示宽度，从而造成有些标签不可见）。
1. 插件会添加小工具，所以你可以通过 '外观'->'小工具'来添加到侧边栏上。打开小工具可以对插件进行设置(背景颜色, 大小, 等).
...
* Pratul Kalia and Bj鰎n Jacob have ported it to [Drupal](http://drupal.org/project/cumulus).
